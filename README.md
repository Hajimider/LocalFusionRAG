...
本地文档问答不仅需要“大模型能回答”，还要解决关键词与语义表达不一致、候选片段排序不准、问题超出知识库范围、索引更新冲突和回答无法追溯等问题。项目将任务拆成文档处理、检索、排序、拒答、生成和评测六个环节，每一步都保留可检查输出。

| 问题 | 项目处理方式 | 可检查的输出 |
| --- | --- | --- |
| 只用关键词检索难以召回语义近似表达 | BGE/FAISS 与中文 BM25 双路召回，再使用 RRF 融合 | Dense、BM25、Hybrid 消融指标 |
| 初次召回顺序不等于问答相关性 | CrossEncoder 联合编码问题和片段并重新排序 | 重排分数、来源顺序、Reranker 状态 |
| 知识库没有答案时模型仍可能编造 | 在校准集选择最低重排分数阈值，低相关问题直接拒答 | 拒答阈值、混淆矩阵、独立测试指标 |
| 文档更新期间可能读到不完整索引 | SHA-256 校验、版本目录、原子指针切换和跨进程文件锁 | `manifest.json`、`CURRENT`、索引版本目录 |
| 大模型回答难以核对来源 | 上下文按资料编号组织，页面返回文件、页码和检索方式 | 回答引用、来源片段、NDJSON 事件 |

项目不把大模型参数记忆当作知识库事实。文档之外的信息不会因为模型“知道”就视为有依据，低相关问题应返回资料不足。
